...
from graphxplore.Dashboard import MetadataDistributionPlotter, DashboardBuilder, HistogramYScaleType
from graphxplore.MetaDataHandling import VariableInfo, DataType, VariableType, MetaData
...
def test_queries():
    if not neo4j_test_config.RUN_DB_TESTS:
        ...
        if not neo4j_test_config.test_connectivity():
            pytest.fail(
                'Neo4J DBMS for testing not available under given configuration. Check "test/neo4j_test_config.py"')
        else:
            ...
            GraphDatabaseWriter.write_graph(
                db_name, graph, overwrite=True, address=address, auth=neo4j_test_config.NEO4J_AUTH)
            ...
            builder = DashboardBuilder(meta, 'root', db_name, address=address, auth=neo4j_test_config.NEO4J_AUTH)
            ...
            builder = DashboardBuilder(meta, 'first_child', db_name, address=address, auth=neo4j_test_config.NEO4J_AUTH)
            with pytest.raises(AttributeError) as exc:
            ...
            builder = DashboardBuilder(
                meta, 'root', db_name, groups=groups, address=address, auth=neo4j_test_config.NEO4J_AUTH)
